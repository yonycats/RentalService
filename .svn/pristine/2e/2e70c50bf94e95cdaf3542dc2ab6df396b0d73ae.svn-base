...
<main class="container">
	<div id="mb-confirm">
		...
		<form name="fmemberconfirm"
			action="<%=request.getContextPath()%>/company/checkPass.do"
			method="post">
			<input type="hidden" name="mb-id" value="<%=cv.getComId()%>" />
			<fieldset>
				회원아이디 <span id="mb-confirm-id"><%=cv.getComId()%></span> <label
				...
				<button type="button" id="com-pass-check" class="btn-lsmall"
...
<script>
		$('#com-pass-check').on('click', function() {
			var comId = "<%=cv.getComId()%>";
			var comPass = $('#confirm-mb-password').val();
            $.ajax({
                url: "<%=request.getContextPath()%>/company/checkDelete.do", // 서버의 URL
                type: 'POST',
                data: { comId: comId,
                		comPass: comPass	
                },
                ...
                success: function(data) {
					if(data.msg == 'success'){
						window.location.href = "<%=request.getContextPath()%>/company/deleteInfo.do";
										} else {
											alert('로그인실패')
										}
