...
<main id="container" class="col-sm-9 mypage-main">
	<div class="mypage-main-top" style="margin-bottom: 150px;">
		...
		<div class="container">
    <form action="<%=request.getContextPath() %>/itemInquiry/update.do?inq=<%=iiv.getInqId() %>" method="POST">
	
	<div class="form-group">
        <label for="title">상품문의제목</label>
        <input type="text" class="form-control" placeholder="title" id="title" name="inqTitle" value="<%=iiv.getInqTitle() %>">
      </div>

	<div class="form-group">
        <label for="title">문의내용</label>
        <input type="text" class="form-control" placeholder="title" id="title" name="inqContent" value="<%=iiv.getInqContent() %>">
      </div>
  ...
  $("#summernote").summernote('code', '<%=iiv.getInqImage() %>');
...
//	 	alert("모든 입력값을 직렬화처리 후 전달예정입니다.");
		...
		$.ajax({
			type: 'post',
			url: '/itemInquiry/update.do',
			data: formData, //url인코딩(%인코딩)
			success:function(res){
				 console.log("응답>>", res);
				 if(res == 1){
					 if(confirm("")) location.href = "/itemInquiry/list.do";
				 }
			},
			error:function(xhr){
